...
import { localState } from "@modules/owlet";
import { logType } from "@lib/logService";
...
export class VCService {
  ...
  private async createHub(vc: VoiceState) {
    ...
    const botIds = localState.controller.getBotIds();
    ...
    state.log.log(embed, vc.guild.id, logType.BOT);
  }
  ...
  public async disbandVC(
    ...
    const users = mainRoom?.permissionOverwrites.cache
      .filter(
        (x) =>
          !localState.controller.getBotIds().includes(x.id) &&
