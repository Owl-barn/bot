...
export async function registerButtons(): Promise<Collection<string, RavenButton>> {
    console.log(" > Loading buttons".green.bold);
    const buttons = new Collection() as Collection<string, RavenButton>;
    ...
    for (const file of files) {
        ...
        if (buttons.get(button.name) !== undefined) {
            console.log(`duplicate commands with name: ${button.name}`.red.bold);
            process.exit();
        }
        ...
        // Log.
        console.log(`${" - Loaded button:".cyan.italic} ${button.disabled ? button.name.red.italic : button.name.green.italic}`);
    }
